    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-sm text-rootstock-orange hover:text-rootstock-orange-dark dark:text-rootstock-orange-light dark:hover:text-rootstock-orange transition-colors"
        >
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          {selectedTokens.map((token) => (
            <div key={token.address} className="flex justify-between items-center p-4 bg-gray-50 dark:bg-dark-tertiary rounded-xl">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full flex items-center justify-center bg-white border border-gray-200 dark:bg-dark-secondary dark:border-dark-tertiary p-1">
                  <img
                    src={token.logoUrl}
                    alt={token.symbol}
                ...
                <div>
                  <p className="font-medium dark:text-white">{token.symbol}</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">{token.name}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="font-semibold dark:text-white">{token.amount || '0'}</p>
        ...
        <div className="space-y-4">
          ...
          <div className="p-6 card-gradient rounded-xl border-2 border-rootstock-orange/30 dark:from-dark-secondary dark:to-black dark:border-rootstock-orange/20">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-lg dark:text-white">RIF Token</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Automatically staked</p>
              </div>
            </div>
            <div className="text-right">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                ...
                {quotes.length > 0 && !quotesLoading && parseFloat(totalRIF) > 0 ? (
                  <span className="text-green-600 dark:text-green-400 ml-1">• Real-time</span>
                ) : quotesError ? (
                  <span className="text-red-500 dark:text-red-400 ml-1">• Quote failed</span>
                ) : quotesLoading ? (
                  <span className="text-blue-500 dark:text-blue-400 ml-1">• Loading...</span>
                ) : (
                  <span className="text-amber-600 dark:text-amber-400 ml-1">• Estimated</span>
                )}
              </p>
              {totalPriceImpact > 0 && (
      ...
      {/* Real-time Quote Details */}
      {
        selectedTokens.length > 0 && (
          <div className="p-4 bg-blue-50 border border-blue-200 dark:bg-blue-900/10 dark:border-blue-900/30 rounded-xl">
            ...
            {quotesError && (
              <div className="mb-3 p-2 bg-red-50 border border-red-200 dark:bg-red-900/10 dark:border-red-900/30 rounded text-red-600 dark:text-red-300 text-sm">
                  ...
                  <div key={token.address} className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-medium text-blue-700 dark:text-blue-300">
                      {quotesLoading ? (
                        ...
                        <span className="text-red-500 dark:text-red-400" title={quote.error}>Quote failed</span>
                      ) : (
                        <span className="text-gray-400 dark:text-gray-400">No quote</span>
                      )}
                    </span>
                  </div>
                );
            ...
            {quotes.length > 0 && !quotesLoading && (
              <div className="mt-3 pt-3 border-t border-blue-200 dark:border-blue-900/30 flex justify-between items-center font-semibold text-blue-800 dark:text-blue-300">
      ...
      {
        showAdvanced && (
          <div className="p-4 bg-gray-50 dark:bg-dark-tertiary rounded-xl space-y-4">
            <h4 className="font-medium text-gray-700 dark:text-gray-200">Advanced Settings</h4>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex gap-2">
                  {['0.1', '0.5', '1.0'].map((value) => (
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg text-sm transition-colors ${slippage === value
                        ...
                        : 'bg-white border border-gray-200 hover:border-rootstock-orange dark:bg-dark-secondary dark:border-dark-tertiary dark:text-gray-300 dark:hover:border-rootstock-orange'
                        }`}
                    >
                      {value}%
                    </button>
                  ...
                  <input
                    ...
                    className="w-20 px-3 py-2 border border-gray-200 rounded-lg text-sm dark:bg-dark-secondary dark:border-dark-tertiary dark:text-white"
                    step="0.1"
                    min="0.1"
                    max="50"
                  />
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 bg-white p-2 rounded-lg border border-gray-200 dark:bg-dark-secondary dark:border-dark-tertiary dark:text-gray-300">
      ...
      {/* Staking Info */}
      <div className="bg-green-50 border border-green-200 dark:bg-green-900/10 dark:border-green-900/30 rounded-xl p-6">
        ...
        <div className="grid md:grid-cols-3 gap-4 text-sm">
          <div>
            <p className="font-medium text-green-700 dark:text-green-300">APY</p>
            <p className="text-green-600 dark:text-green-400">{estimatedAPY}%</p>
          </div>
          <div>
            <p className="font-medium text-green-700 dark:text-green-300">Lock Period</p>
            <p className="text-green-600 dark:text-green-400">Flexible</p>
          </div>
          <div>
            <p className="font-medium text-green-700 dark:text-green-300">Rewards</p>
            <p className="text-green-600 dark:text-green-400">Daily</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex justify-between text-sm">
          ...
          <span className="dark:text-gray-200">1 rUSDT ≈ 17.11 RIF</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-gray-600 dark:text-gray-400">Slippage Tolerance</span>
          <span className="dark:text-gray-200">{slippage}%</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-gray-600 dark:text-gray-400">Estimated Gas</span>
          <span className="dark:text-gray-200">{calculateGasFee()} RBTC</span>
        </div>
        <div className="flex justify-between text-sm font-medium pt-2 border-t dark:border-white/10">
          <span className="dark:text-gray-200">You'll stake</span>
          <span className="text-rootstock-orange font-bold">{estimatedRIF.toFixed(2)} tRIF</span>
        </div>
      </div>

      {/* Chain-specific Transaction Info */}
      <div className="p-4 rounded-xl border bg-blue-50 border-blue-200 dark:bg-blue-900/10 dark:border-blue-900/30">
        <div className="flex items-center gap-2">
      ...
      {/* Approval Progress for Rootstock */}
      {
        !isEIP5792 && needsApprovals && approvalStep > 0 && (
          <div className="p-4 rounded-xl border bg-yellow-50 border-yellow-200 dark:bg-yellow-900/10 dark:border-yellow-900/30 overflow-hidden">
            {(() => {
              ...
              return (
                <>
                  ...
                  <p className="text-xs mt-2 text-yellow-600 dark:text-yellow-500">
      ...
      {
        error && (
          <div className="flex items-center justify-between p-4 bg-red-50 border border-red-200 dark:bg-red-900/10 dark:border-red-900/30 rounded-xl">
            <div className="flex items-center gap-2">
...

      {
        (hash || batchId) && !isConfirmed && (
          <div className="flex items-center gap-2 p-4 bg-blue-50 border border-blue-200 dark:bg-blue-900/10 dark:border-blue-900/30 rounded-xl">
      ...
      {
        !hasValidAmounts && (
          <div className="flex items-center gap-2 p-4 bg-amber-50 border border-amber-200 dark:bg-amber-900/10 dark:border-amber-900/30 rounded-xl">
      ...
      <div className="text-center text-sm text-gray-500 dark:text-gray-400">
        ...
        <p className={`text-xs mt-2 ${chainId === 84532 ? 'text-green-600 dark:text-green-400' : 'text-blue-600 dark:text-blue-400'}`}>
          {chainId === 84532
